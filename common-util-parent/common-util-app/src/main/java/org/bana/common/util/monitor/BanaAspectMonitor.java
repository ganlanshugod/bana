...
public abstract class BanaAspectMonitor {
	private static final Logger LOG = LoggerFactory.getLogger(BanaAspectMonitor.class);
	...
	public Object around(ProceedingJoinPoint jp) throws Throwable {
		LOG.debug(this.getClass() + "=========AOP execute ========");
		// 获取访问参数，记录访问时间
		...
		LOG.debug("execute class function is    " + jp.getSignature().getDeclaringTypeName() + "===="
		...
		try {
			...
			LOG.debug(this.getClass() + "==============monitor excute success ===============");
			return proceed;
		} catch (Throwable e) {
			LOG.debug(this.getClass() + "==============monitor find exception=================", e);
			// 获取异常信息
			...
			LOG.debug("execute time is " + (monitorLog.getEndDate().getTime() - monitorLog.getBeginDate().getTime())
					+ "ms");
			// 对访问日志进行记录
	...
	private void saveMonitorLog(BanaAspectMonitorLog monitorLog) {
		...
		pool.execute(new MapRunable(map) {
			@Override
			public void run() {
				BanaAspectMonitorLog monitorLog = (BanaAspectMonitorLog) map.get("monitorLog");
