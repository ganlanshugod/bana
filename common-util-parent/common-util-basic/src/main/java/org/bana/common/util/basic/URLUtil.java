...
public class URLUtil {
	...
	/**
	 * @Description: * 功能：检测当前URL是否可连接或是否有效, 描述：最多连接网络 3 次, 如果 3 次都不成功，视为该地址不可用
	 * @author Liu Wenjie
	...
	public static int isConnect(String urlStr) {
		...
		while (counts < REQUEST_COUNT) {
			...
			try {
				...
				LOG.debug("请求断开的URL一次需要：" + (System.currentTimeMillis() - start) + "毫秒");
				if (responseCode == 200) {
					LOG.debug(urlStr + "--可用，返回的应答编码为" + responseCode);
				} else {
					LOG.debug(urlStr + "--不可用，返回的应答编码为" + responseCode);
				}
				break;
			} catch (Exception ex) {
				counts++;
				LOG.debug("请求断开的URL一次需要：" + (System.currentTimeMillis() - start) + "毫秒");
				LOG.debug("连接第 " + counts + " 次，" + urlStr + "--不可用");
				continue;
			} finally {
				if (conn != null) {
					conn.disconnect();
