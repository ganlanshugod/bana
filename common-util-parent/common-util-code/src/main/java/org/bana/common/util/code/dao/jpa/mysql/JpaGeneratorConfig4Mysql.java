package org.bana.common.util.code.dao.jpa.mysql;
...
import org.bana.common.util.code.dao.jpa.JPAColumnHandler;
...
public class JpaGeneratorConfig4Mysql extends MybatisGeneratorConfig {
	...
	*/ 
	...
	private JPAColumnHandler columnHandler = new JPAColumnHandler();
	...
	*/ 
	private Column initTableColumns(ResultSet rs, boolean needRemove) {
		try {
			...
			column.setColumnHandler(this.columnHandler);
			return column;
		} catch (Exception e) {
	...
	* @Description: 添加
	* @author liuwenjie   
	* @date Sep 22, 2020 1:27:36 PM 
	* @param json
	* @param class1
	* @param class2  
	*/ 
	public void addColumnCover(ColumnType json, Class<?> coverJavaClass, Class<?> coverClass) {
		ColumnCover cover = new ColumnCover(coverClass.getName(), json.toString(), coverJavaClass.getName());
		columnHandler.addCover(cover);
	}
