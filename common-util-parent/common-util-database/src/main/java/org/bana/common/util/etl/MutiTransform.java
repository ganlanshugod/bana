...
public class MutiTransform extends SimpleTransform {
	...
	public void execute() throws Exception {
		...
		if (sourceGroup != null) {
			...
			if (conditions != null) {
				for (Condition conditionCond : conditions) {
					...
					if (StringUtils.isBlank(conditionCond.getType())) {
						...
						condition.put(conditionCond.getName(),
								new BasicDBObject(conditionCond.getType(), conditionCond.getValue()));
					}
				}
			}
			...
			BasicDBObjectBuilder group = BasicDBObjectBuilder.start().add("ns", etlConfig.getSourceTableName())
					.add("$keyf", sourceGroup.getKeyf()).add("cond", condition).add("$reduce", reduce)
					.add("initial", initial);
			CommandResult res = db.command(new BasicDBObject("group", group.get()));
			res.throwOnError();
			// BasicDBList sourceList = (BasicDBList)sourceCollection.group(keyf, condition,
			// initial, reduce);
			...
			PreparedStatement destPrest = destConn.prepareStatement(insertSql, ResultSet.TYPE_SCROLL_SENSITIVE,
			...
			destConn.commit();
			destPrest.close();
			LOG.debug("transform table success..., count is " + count + ", total second is "
	 ...
	 * @param i
	 */
	private void excuteMongoInsert(Connection destConn, BasicDBObject rs, PreparedStatement preStat, int count)
			throws Exception {
		// 设置参数
				...
				case STRING:
					...
					if (!StringUtils.isBlank(value)) {
						if (!StringUtils.isBlank(etlConfig.getSourceEncoding(), etlConfig.getDestEncoding())) {
							value = new String(value.getBytes(etlConfig.getSourceEncoding()),
									etlConfig.getDestEncoding());
						}
