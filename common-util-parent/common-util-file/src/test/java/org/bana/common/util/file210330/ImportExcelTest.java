...
//	private String fileName = "31 20210327 WP-20071989-BC-01.xlsx";
	...
	private String insertPaySql = "insert into BiomedicalOrderPaymentInfo(Inumid "
			...
			+ "      ,ExperimentContent"
			+ "      ,ReceivedPayment"
			+ "      ,RefundReceivedPayment"
			+ "      ,GuaranteePayment"
			+ "      ,RefundGuaranteePayment"
			+ "      ,RemoveGuaranteePayment"
			+ "      ,RefundTotal"
			+ "      ,CreditType"
			+ "      ,CreateTime"
			+ "      ,GuaranteeDueDate)"
...
//			}
	...
	public void importFile(File file) throws Exception {
		List<Integer> indexList = Arrays.asList(targetRowTitle).stream().map(item -> rowTitleList.indexOf(item)).collect(Collectors.toList());
...
//				System.out.print(rowTitle[targetIndex] + " : " + cellValue + "  ");
		...
		LOG.info("读取excel完成，完成到" + currentRowNum);
		fis.close();
